...
public class CanSeeConditionSnake : Condition
{
    ...
    public override bool Test(FiniteStateMachine fsm)
    {
        ...
        if (direction.magnitude < viewDistance)
        {
            ...
            if (angle < viewAngle)
            {
                Debug.Log($"hahahahbashgasdyufsdyfgyisdgfysdrfgyi");
                ...
                {
                    float angle2 = Vector3.Angle(direction2.normalized, fsm.GetNavMeshAgent().target.forward);
                    if (angle2 < viewAngle)
                    {
                        Debug.Log($"iTS TRUEEEEEEE");
                        return !negation;
                    }
                }
            }
